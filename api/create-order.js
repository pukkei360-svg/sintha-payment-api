...
export default async function handler(request) {
  ...
  if (!keyId || !keySecret) return json({ error: "server_not_configured", detail: "Missing Razorpay keys" }, 500);
  let uid;
  try {
    const auth = request.headers.get("Authorization") || "";
    const match = auth.match(/^Bearer (.+)$/);
    if (!match) return json({ error: "unauthenticated", detail: "No Bearer token" }, 401);
    const payload = await verifyFirebaseToken(match[1], projectId);
    uid = payload.sub;
  } catch (e) { console.error("Auth error:", e.message); return json({ error: "unauthenticated", detail: e.message }, 401); }
  try {
    const body = await request.text();
    let amount;
    try { amount = JSON.parse(body).amount; } catch (e) { return json({ error: "invalid_json", detail: "Could not parse request body" }, 400); }
    const amountRupees = Number(amount);
    if (!amountRupees || amountRupees <= 0) return json({ error: "invalid_amount", detail: `Got: ${amount}` }, 400);
    const authB64 = base64Encode(`${keyId}:${keySecret}`);
    const res = await fetch("https://api.razorpay.com/v1/orders", {
      method: "POST",
      headers: { Authorization: `Basic ${authB64}`, "Content-Type": "application/json" },
      body: JSON.stringify({ amount: Math.round(amountRupees * 100), currency: "INR", receipt: `receipt_${uid}_${Date.now()}`, payment_capture: 1, notes: { uid } }),
    });
    const orderText = await res.text();
    console.log("Razorpay response:", res.status, orderText);
    if (!res.ok) {
